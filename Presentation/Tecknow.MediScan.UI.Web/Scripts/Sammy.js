...
(function(factory) {
    ...
    var sammy,
        pathReplacer = "([^\/]+)",
        pathNameMatcher = /:([\w\d]+)/g,
        queryStringMatcher = /\?([^#]*)?$/,
        // mainly for making `arguments` an Array
        makeArray = function(nonarray) { return Array.prototype.slice.call(nonarray); },
    ...
    $.extend(sammy.Object.prototype, {
        ...
        keys: function(attributesOnly) {
            var keys = [];
            for (var property in this) {
                if (!isFunction(this[property]) || !attributesOnly) {
                    keys.push(property);
                }
            }
            return keys;
        ...
        toString: function(includeFunctions) {
            var s = [];
            $.each(this, function(k, v) {
                if (!isFunction(v) || includeFunctions) {
                    s.push("\"" + k + "\": " + v.toString());
                }
            });
    ...
    // hash.
    sammy.DefaultLocationProxy = function(app, runIntervalEvery) {
        this.app = app;
        // set is native to false and start the poller immediately
        this.is_native = false;
        this.has_history = hasHistory;
        this._startPolling(runIntervalEvery);
    };

    sammy.DefaultLocationProxy.fullPath = function(locationObj) {
        // Bypass the `window.location.hash` attribute.  If a question mark
        // appears in the hash IE6 will strip it and all of the following
        // characters from `window.location.hash`.
        var matches = locationObj.toString().match(/^[^#]*(#.+)$/);
        var hash = matches ? matches[1] : "";
        return [locationObj.pathname, locationObj.search, hash].join("");
    };
    $.extend(sammy.DefaultLocationProxy.prototype, {
        // bind the proxy events to the current app.
        bind: function() {
            var proxy = this, app = this.app, lp = sammy.DefaultLocationProxy;
            $(window).bind("hashchange." + this.app.eventNamespace(), function(e, nonNative) {
                // if we receive a native hash change event, set the proxy accordingly
                // and stop polling
                if (proxy.is_native === false && !nonNative) {
            ...
            if (hasHistory && !app.disable_push_state) {
                // bind to popstate
                ...
                $(document).delegate("a", "click.history-" + this.app.eventNamespace(), function(e) {
                    ...
                    if (hostname == window.location.hostname &&
                        app.lookupRoute("get", fullPath) &&
                        sammy.targetIsThisWindow(e, "a")) {
                        e.preventDefault();
                        proxy.setLocation(fullPath);
                        return false;
                    }
                });
            }
        ...
        setLocation: function(newLocation) {
            if (/^([^#\/]|$)/.test(newLocation)) { // non-prefixed url
                if (hasHistory && !this.app.disable_push_state) {
            ...
            if (newLocation != this.getLocation()) {
                ...
                if (hasHistory && !this.app.disable_push_state && /^\//.test(newLocation)) {
                    history.pushState({ path: newLocation }, window.title, newLocation);
        ...
        _startPolling: function(every) {
            ...
            if (!sammy.DefaultLocationProxy._interval) {
                ...
                var hashCheck = function() {
                    var currentLocation = proxy.getLocation();
                    if (typeof sammy.DefaultLocationProxy._last_location == "undefined" ||
                        currentLocation != sammy.DefaultLocationProxy._last_location) {
                        window.setTimeout(function() {
                            $(window).trigger("hashchange", [true]);
                        }, 0);
                    }
                    sammy.DefaultLocationProxy._last_location = currentLocation;
                };
                hashCheck();
    ...
    sammy.Application = function(appFunction) {
        var app = this;
        this.routes = {};
        ...
        if (isFunction(appFunction)) {
            appFunction.apply(this, [this]);
        }
    ...
    sammy.Application.prototype = $.extend({}, sammy.Object.prototype, {
        ...
        use: function() {
            ...
            try {
                ...
                if (typeof plugin == "string") {
                    pluginName = "Sammy." + plugin;
                    plugin = sammy[plugin];
                }
                plugin.apply(this, args);
            } catch (e) {
                if (typeof plugin === "undefined") {
                    this.error("Plugin Error: called use() but plugin (" + pluginName.toString() + ") is not defined", e);
                } else if (!isFunction(plugin)) {
                    this.error("Plugin Error: called use() but '" + pluginName.toString() + "' is not a function", e);
                } else {
        ...
        //        });
        //
        setLocationProxy: function(newProxy) {
            var originalProxy = this._location_proxy;
            this._location_proxy = newProxy;
            if (this.isRunning()) {
                if (originalProxy) {
                    // if there is already a location proxy, unbind it.
                    originalProxy.unbind();
                }
        ...
        route: function(verb, path) {
            var app = this, paramNames = [], addRoute, pathMatch, callback = Array.prototype.slice.call(arguments, 2);

            // if the method signature is just (path, callback)
            // assume the verb is 'any'
            if (callback.length === 0 && isFunction(path)) {
                callback = [path];
                path = verb;
                verb = "any";
            }
            ...
            if (path.constructor == String) {
                ...
                pathNameMatcher.lastIndex = 0;

                // find the names
                while ((pathMatch = pathNameMatcher.exec(path)) !== null) {
                    paramNames.push(pathMatch[1]);
                }
                // replace with the path replacement
                path = new RegExp(path.replace(pathNameMatcher, pathReplacer) + "$");
            }
            // lookup callbacks
            ...
            addRoute = function(withVerb) {
                var r = { verb: withVerb, path: path, callback: callback, param_names: paramNames };
                // add route to routes array
                app.routes[withVerb] = app.routes[withVerb] || [];
                // place routes in order of definition
                app.routes[withVerb].push(r);
            };

            if (verb === "any") {
                $.each(this.ROUTE_VERBS, function(i, v) { addRoute(v); });
            } else {
                addRoute(verb);
            }
        ...
        // Alias for route('get', ...)
        get: routeWrapper("get"),

        // Alias for route('post', ...)
        post: routeWrapper("post"),

        // Alias for route('put', ...)
        put: routeWrapper("put"),

        // Alias for route('delete', ...)
        del: routeWrapper("delete"),

        // Alias for route('any', ...)
        any: routeWrapper("any"),
        ...
        //      });
        //
        mapRoutes: function(routeArray) {
            var app = this;
            $.each(routeArray, function(i, routeArgs) {
                app.route.apply(app, routeArgs);
            });
            return this;
        },
        ...
        bind: function(name, data, callback) {
            ...
            this.listeners[name].push(listenerCallback);
            if (this.isRunning()) {
                // if the app is running
                // *actually* bind the event to the app element
                this._listen(name, listenerCallback);
            }
            return this;
        },
        ...
        run: function(startUrl) {
            ...
            $.each(this.listeners.toHash(), function(name, callbacks) {
                $.each(callbacks, function(i, listenerCallback) {
                    app._listen(name, listenerCallback);
                });
            });

            this.trigger("run", { start_url: startUrl });
            this._running = true;
            // set last location
            this.last_location = null;
            if (!(/\#(.+)/.test(this.getLocation())) && typeof startUrl != "undefined") {
                this.setLocation(startUrl);
            }
            // check url
            this._checkLocation();
            this._location_proxy.bind();
        ...
        unload: function() {
            ...
            $.each(this.listeners.toHash(), function(name, listeners) {
                $.each(listeners, function(i, listenerCallback) {
                    app._unlisten(name, listenerCallback);
                });
            });
            this._running = false;
            return this;
        ...
        routablePath: function(path) {
            return path.replace(queryStringMatcher, "");
        },
        ...
        runRoute: function(verb, path, params, target) {
            ...
            if (route) {
                ...
                if ((pathParams = route.path.exec(this.routablePath(path))) !== null) {
                    // first match is the full path
                    pathParams.shift();
                    // for each of the matches
                    $.each(pathParams, function(i, param) {
                        // if theres a matching param name
                        if (route.param_names[i]) {
                            // set the name to the match
                            params[route.param_names[i]] = decode(param);
                        } else {
                            // initialize 'splat'
                            if (!params.splat) {
                                params.splat = [];
                            }
                            params.splat.push(decode(param));
                        }
                    });
                }
                ...
                if (params.splat) {
                    callbackArgs = callbackArgs.concat(params.splat);
                }
                ...
                wrappedRoute = function() {
                    ...
                    if (route.callback && route.callback.length) {
                        ...
                        nextRoute = function() {
                            i++;
                            if (route.callback[i]) {
                                returned = route.callback[i].apply(context, callbackArgs);
                            } else if (app._onComplete && typeof (app._onComplete === "function")) {
                                app._onComplete(context);
                            }
                        };
                        callbackArgs.push(nextRoute);
                        nextRoute();
                    }
                ...
                $.each(arounds.reverse(), function(i, around) {
                    var lastWrappedRoute = wrappedRoute;
                    wrappedRoute = function() { return around.apply(context, [lastWrappedRoute]); };
                });
                try {
                    finalReturned = wrappedRoute();
                } catch (e) {
        ...
        //
        contextMatchesOptions: function(context, matchOptions, positive) {
            ...
            var pathMatched = true, verbMatched = true;
            if (options.path) {
                if (!isRegExp(options.path)) {
                    options.path = new RegExp(options.path.toString() + "$");
                }
                pathMatched = options.path.test(context.path);
            }
            if (options.verb) {
                if (typeof options.verb === "string") {
                    verbMatched = options.verb === context.verb;
                } else {
                    verbMatched = options.verb.indexOf(context.verb) > -1;
                }
            }
            return positive ? (verbMatched && pathMatched) : !(verbMatched && pathMatched);
        },
        ...
        templateCache: function(key, value) {
            if (typeof value != "undefined") {
                return templateCache[key] = value;
            } else {
        ...
        error: function(message, originalError) {
            ...
            this.trigger("error", { message: originalError.message, error: originalError });
        ...
        _parseFormParams: function($form) {
            ...
            for (i = 0; i < formFields.length; i++) {
                params = this._parseParamPair(params, formFields[i].name, formFields[i].value);
            }
            return params;
        },
        ...
        _parseQueryString: function(path) {
            ...
            parts = path.match(queryStringMatcher);
            if (parts && parts[1]) {
                pairs = parts[1].split("&");
                for (i = 0; i < pairs.length; i++) {
                    pair = pairs[i].split("=");
                    params = this._parseParamPair(params, decode(pair[0]), decode(pair[1] || ""));
                }
            }
            return params;
        },
    ...
    //
    sammy.RenderContext = function(eventContext) {
    ...
    sammy.RenderContext.prototype = $.extend({}, sammy.Object.prototype, {
        ...
        load: function(location, options, callback) {
            ...
            return this.then(function() {
                var shouldCache, cached, isJson, locationArray;
                if (isFunction(options)) {
                    callback = options;
                    options = {};
                } else {
                ...
                if (typeof location === "string") {
                    // it's a path
                    isJson = (location.match(/\.json(\?|$)/) || options.json);
                    shouldCache = isJson ? options.cache === true : options.cache !== false;
                    ...
                    if (shouldCache && (cached = this.event_context.app.templateCache(location))) {
                        return cached;
                    }
                    this.wait();
                    $.ajax($.extend({
        ...
        renderEach: function(location, name, data, callback) {
            ...
            return this.load(location).then(function(content) {
                var rctx = this;
                if (!data) {
                    data = isArray(this.previous_content) ? this.previous_content : [];
                }
                if (callback) {
    ...
    sammy.EventContext.prototype = $.extend({}, sammy.Object.prototype, {
        ...
        $element: function() {
            return this.app.$element(makeArray(arguments).shift());
        },
        ...
        redirect: function() {
            var to,
                args = makeArray(arguments),
                currentLocation = this.app.getLocation(),
                l = args.length;
            if (l > 1) {
                var i = 0, paths = [], pairs = [], params = {}, hasParams = false;
                for (; i < l; i++) {
            ...
            if (new RegExp(to).test(currentLocation)) {
                this.app.trigger("location-changed");
            }
        },
